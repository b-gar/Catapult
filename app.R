...
server <- function(input, output, session) {
      ...
      validate(
        ...
        for (i in 1:length(input$files$name)) {
            ...
            colnames(dfCombined) <- c("Name", "Position", "Duration", "Distance", "playerLoad", "maxVelocity", "Date", 
                                      "Activity", "gameCode")
            dfCombined <- dfCombined %>% filter(Distance != 0 & playerLoad != 0)
    ...
    output$PlayerEWMA <- renderPlotly({
      ...
      p7 <- Data() %>% filter(Name == input$player) %>%
        ...
        geom_hline(aes(yintercept = input$acwr[1])) + geom_hline(aes(yintercept = input$acwr[2])) + ggtitle("Player Acute/Chronic Workload Ratio") + 

...
server <- function(input, output, session) {
    ...
    output$notificationLow <- renderMenu({
      ...
      if (nrow(nmLow) > 0) {
        ...
        dropdownMenu(type = "notifications", .list = notifMessageL, icon = icon("long-arrow-alt-down fa-3x"), badgeStatus = "warning")
      }
      else(
    ...
    output$notificationHigh <- renderMenu({
      ...
      if (nrow(nmHigh) > 0) {
        ...
        dropdownMenu(type = "notifications", .list = notifMessageH, icon = icon("long-arrow-alt-up fa-3x"), badgeStatus = "danger")
      }
      else(
